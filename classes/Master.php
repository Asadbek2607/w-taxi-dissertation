...
Class Master extends DBConnection {
    ...
    function save_booking(){
        ...
        $data = "";
        foreach($_POST as $k =>$v){
            if(!in_array($k,array('id', 'pickup_date'))){
                if(!empty($data)) $data .=",";
                $data .= " `{$k}`='{$v}' ";
            }
        }
        if(empty($id)){
            $sql = "INSERT INTO `booking_list` set `pickup_date`='{$pickup_date}', {$data} ";
            $save = $this->conn->query($sql);
        }else{
            $sql = "UPDATE `booking_list` set `pickup_date`='{$pickup_date}', {$data} where id = '{$id}' ";
            $save = $this->conn->query($sql);
        }
        if($save){
            $resp['status'] = 'success';
